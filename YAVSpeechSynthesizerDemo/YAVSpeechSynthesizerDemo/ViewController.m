...
- (void)viewDidLoad {
    ...
    [[YSpeechManager shareSpeechManager] startSpeechWith:self.textView.text];
...
- (void)speechSynthesizer:(AVSpeechSynthesizer *)synthesizer didFinishSpeechUtterance:(AVSpeechUtterance *)utterance {
    NSLog(@"%s",__func__);
    _speechCount++;
    if (_speechCount < _dataArray.count) {
        AVSpeechUtterance *speechUtterance = [[AVSpeechUtterance alloc] initWithString:_dataArray[_speechCount]];
        AVSpeechSynthesisVoice *voiceType = [AVSpeechSynthesisVoice voiceWithLanguage:@"zh-CN"];
        speechUtterance.voice = voiceType;
        speechUtterance.pitchMultiplier = 1;
        [_speechSynthesizer speakUtterance:speechUtterance];
